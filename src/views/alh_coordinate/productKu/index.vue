    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="客户">
          <el-input v-model="listQuery.filter" placeholder="请输入客户" />
        </el-form-item>
        <el-form-item label="选材">
          <el-input v-model="listQuery.filter" placeholder="请输入选材" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="客户" prop="index" align="center" min-width="50">
          <template slot-scope="{ row }">
            <span>{{ '测试'+row.index }}</span>
          </template>
        </el-table-column>
        <el-table-column label="选材" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ '材料'+row.index }}</span>
          </template>
        </el-table-column>
        <el-table-column label="项目概算" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.money }}</span>
          </template>
        </el-table-column>
        <el-table-column label="报价" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.money }}</span>
          </template>
        </el-table-column>
