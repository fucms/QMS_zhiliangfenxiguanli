  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="抽样方案序列号" prop="index" align="center" min-width="50">
          <template slot-scope="{ row }">
            <span>{{ row.index }}</span>
          </template>
        </el-table-column>
        <el-table-column label="批量下限" prop="index" align="center" min-width="50">
          <template slot-scope="{ row }">
            <span>{{ row.number }}</span>
          </template>
        </el-table-column>
        <el-table-column label="批量上限" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.number }}</span>
          </template>
        </el-table-column>
        <el-table-column label="特殊检验水平S-1" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.checkLevel }}</span>
          </template>
        </el-table-column>
        <el-table-column label="特殊检验水平S-2" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.checkLevel }}</span>
          </template>
        </el-table-column>
        <el-table-column label="特殊检验水平S-3" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.checkLevel }}</span>
          </template>
        </el-table-column>
        <el-table-column label="一般检验水平1" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.checkLevel }}</span>
          </template>
        </el-table-column>
        <el-table-column label="一般检验水平2" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.checkLevel }}</span>
          </template>
        </el-table-column>
