    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        ...
        <el-form-item label="料号">
          <el-input v-model="listQuery.filter" placeholder="请输入料号" />
        </el-form-item>
        <el-form-item label="品名">
          <el-input v-model="listQuery.filter" placeholder="请输入品名" />
        </el-form-item>
        <el-form-item label="生产厂家">
          <el-input v-model="listQuery.filter" placeholder="请输入生产厂家" />
        </el-form-item>
        <el-form-item label="日期">
          <el-input v-model="listQuery.filter" placeholder="请输入日期" />
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column label="料号" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
        <el-table-column label="品名" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ '品名' + row.index }}</span>
          </template>
        </el-table-column>
        <el-table-column label="计量单位" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ (row.index % 2) == 1 ? 'mm':'cm ' }}</span>
          </template>
        </el-table-column>
        <el-table-column label="生产厂家" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.customerName }}</span>
          </template>
        </el-table-column>
        <el-table-column label="不合格品质" prop="code" align="center">
          <template slot-scope="{ row }">
