    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="型号">
          <el-input v-model="listQuery.filter" placeholder="请输入型号" />
        </el-form-item>
        <el-form-item label="车间">
          <el-input v-model="listQuery.filter" placeholder="请输入车间" />
        </el-form-item>
        <el-form-item label="产线">
          <el-input v-model="listQuery.filter" placeholder="请输入产线" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="型号" prop="index" align="center" min-width="50">
          <template slot-scope="{ row }">
            <span>{{ '型号' + row.index }}</span>
          </template>
        </el-table-column>
        <el-table-column label="车间" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ '车间' + row.index }}</span>
          </template>
        </el-table-column>
        <el-table-column label="产线" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ '产线' + row.index }}</span>
          </template>
        </el-table-column>
        <el-table-column label="物料/产品编号" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.typeNumber }}</span>
          </template>
        </el-table-column>
        <el-table-column label="记录员" prop="code" align="center">
          <template slot-scope="{ row }">
